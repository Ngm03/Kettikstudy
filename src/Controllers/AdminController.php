...
class AdminController
{
    ...
    public function sendBroadcast()
    {
        ...
        $query = "SELECT DISTINCT sl.user_id FROM study_leads sl WHERE sl.user_id IS NOT NULL";
        if ($filter !== 'all') {
            $query .= " AND sl.status = ?";
        ...
        if (empty($recipients) && $filter === 'all') {
            $allUsers = $this->db->query("SELECT id FROM study_users WHERE role != 'admin'")->fetchAll(\PDO::FETCH_COLUMN);
            $recipients = $allUsers;
        }

        if (empty($recipients)) {
            echo json_encode(['success' => true, 'count' => 0, 'message' => 'Нет получателей для данного фильтра']);
            return;
        }

        $notifStmt = $this->db->prepare("INSERT INTO study_notifications (user_id, type, title, body) VALUES (?, 'broadcast', ?, ?)");
        foreach ($recipients as $userId) {
            $notifStmt->execute([$userId, $title, $body]);
        }

        $this->db->prepare("INSERT INTO study_broadcasts (title, body, filter, recipient_count) VALUES (?, ?, ?, ?)")
            ->execute([$title, $body, $filter, count($recipients)]);

        if (!empty($recipients)) {
            try {
                $pushController = new \App\Controllers\PushController();
                $pushController->sendToUsers($recipients, $title, $body);
            } catch (\Exception $e) {
            }
        }
        ...
        if ($adminId) {
            $roomStmt = $this->db->query("SELECT id FROM study_chat_rooms WHERE type = 'broadcast' LIMIT 1");
            $broadcastRoomId = $roomStmt->fetchColumn();

            if (!$broadcastRoomId) {
                $this->db->exec("INSERT INTO study_chat_rooms (type, name) VALUES ('broadcast', '📢 Рассылка')");
                $broadcastRoomId = $this->db->lastInsertId();
            }

            if ($broadcastRoomId) {
                $chatMsg = "📢 **" . $title . "**\n\n" . $body;
                $msgStmt = $this->db->prepare("INSERT INTO study_chat_messages (room_id, user_id, message) VALUES (?, ?, ?)");
                
                $msgStmt->execute([$broadcastRoomId, $adminId, $chatMsg]);
            }
        }

        echo json_encode(['success' => true, 'count' => count($recipients)]);
    }
    ...
    public function getBroadcastHistory()
    {
        ...
        try {
            $stmt = $this->db->query("SELECT * FROM study_broadcasts ORDER BY created_at DESC LIMIT 50");
