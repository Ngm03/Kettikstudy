<style>
    ...
    @media (max-width: 900px) { .ad-stats-grid { grid-template-columns: repeat(2, 1fr); } }
    @media (max-width: 500px) { 
        .ad-stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; } 
        .ad-stat-card { padding: 12px 10px; gap: 8px; }
        .ad-stat-icon { width: 36px; height: 36px; border-radius: 10px; }
        .ad-stat-icon svg { width: 18px; height: 18px; }
        .ad-stat-num { font-size: 1.3rem; }
        .ad-stat-label { font-size: 0.65rem; }
    }
    ...
    .ad-stat-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.08); transform: translateY(-1px); }
    .ad-stat-icon {
    ...
    .ad-stat-icon svg { width: 22px; height: 22px; }
    ...
    .ad-search-wrap input:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.1); }
    ...
    .ad-chip:hover { background: #f1f5f9; border-color: #cbd5e1; }
    .ad-chip.active { background: #1e293b; color: #fff; border-color: #1e293b; }
    ...
    .ad-table { width: 100%; border-collapse: collapse; min-width: 800px; }
    .ad-table th {
        text-align: left;
        padding: 12px 20px;
        background: #f8fafc;
    ...
    .ad-btn-view { background: #eff6ff; color: #3b82f6; }
    .ad-btn-view:hover { background: #dbeafe; transform: scale(1.05); }
    .ad-btn-approve { background: #f0fdf4; color: #22c55e; }
    .ad-btn-approve:hover { background: #dcfce7; transform: scale(1.05); }
    .ad-btn-reject { background: #fef2f2; color: #ef4444; }
    .ad-btn-reject:hover { background: #fee2e2; transform: scale(1.05); }

    .ad-mobile-cards { display: none; padding: 12px; }
    .ad-card {
        background: #fff;
        border: 1px solid #e2e8f0;
        border-radius: 12px;
    ...
    .ad-card-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px; gap: 8px; }
    .ad-card-title { font-weight: 700; font-size: 0.95rem; color: #0f172a; line-height: 1.2; }
    .ad-card-sub { font-size: 0.8rem; color: #64748b; margin-top: 2px; }
    .ad-card-row { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; flex-wrap: wrap; }
    .ad-card-footer { 
    ...
    @media (max-width: 800px) {
        ...
        .ad-mobile-cards { display: block !important; }
        .ad-toolbar { padding: 10px 12px; flex-direction: column; align-items: stretch; }
        .ad-count-badge { align-self: flex-start; margin-left: 0; }
        .ad-filter-chips { overflow-x: auto; padding-bottom: 4px; flex-wrap: nowrap; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
        .ad-filter-chips::-webkit-scrollbar { display: none; }
        .ad-chip { white-space: nowrap; }
    }

    .ad-empty { padding: 60px 20px; text-align: center; }
    .ad-empty svg { width: 48px; height: 48px; color: #cbd5e1; margin-bottom: 12px; display: inline-block; }
    .ad-empty p { color: #64748b; font-size: 0.9rem; font-weight: 500; margin: 0; }

    @keyframes fadeRow { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } }
    .ad-anim-in { animation: fadeRow 0.3s ease; }
</style>

<div class="ad-stats-grid">
    <div class="ad-stat-card">
        <div class="ad-stat-icon" style="background:#f1f5f9; color:#475569;">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        </div>
        <div class="ad-stat-info">
            <div class="ad-stat-num" id="stat-total">—</div>
            <div class="ad-stat-label"><?= __('admin_docs_stat_total') ?></div>
        </div>
    </div>
    <div class="ad-stat-card">
        ...
        <div class="ad-stat-info">
            <div class="ad-stat-num" id="stat-pending">—</div>
            <div class="ad-stat-label"><?= __('admin_docs_stat_pending') ?></div>
        </div>
    </div>
    <div class="ad-stat-card">
        ...
        <div class="ad-stat-info">
            <div class="ad-stat-num" id="stat-approved">—</div>
            <div class="ad-stat-label"><?= __('admin_docs_stat_approved') ?></div>
        </div>
    </div>
    <div class="ad-stat-card">
        ...
        <div class="ad-stat-info">
            <div class="ad-stat-num" id="stat-rejected">—</div>
            <div class="ad-stat-label"><?= __('admin_docs_stat_rejected') ?></div>
        </div>
    </div>
</div>
...
<div class="ad-panel">
    <div class="ad-toolbar">
        <div class="ad-search-wrap">
            ...
            <input type="text" id="ad-search" placeholder="<?= __('admin_docs_search_ph') ?>" oninput="filterTable()">
        </div>
        
        <div class="ad-filter-chips">
            <button class="ad-chip active" data-filter="all" onclick="setFilter(this, 'all')"><?= __('admin_docs_filter_all') ?></button>
            <button class="ad-chip" data-filter="pending" onclick="setFilter(this, 'pending')">
                <span style="width:8px;height:8px;border-radius:50%;background:#f59e0b;"></span> <?= __('admin_docs_filter_pending') ?>
            </button>
            <button class="ad-chip" data-filter="approved" onclick="setFilter(this, 'approved')">
                <span style="width:8px;height:8px;border-radius:50%;background:#10b981;"></span> <?= __('admin_docs_filter_approved') ?>
            </button>
            <button class="ad-chip" data-filter="rejected" onclick="setFilter(this, 'rejected')">
                <span style="width:8px;height:8px;border-radius:50%;background:#ef4444;"></span> <?= __('admin_docs_filter_rejected') ?>
            </button>
        </div>

        <div class="ad-count-badge" id="ad-count-badge">0 / 0</div>
    </div>
    ...
    <div class="ad-mobile-cards" id="ad-mobile-cards"></div>
</div>
...
function setFilter(btn, status) {
    document.querySelectorAll('.ad-chip').forEach(b => b.classList.remove('active'));
...
function filterTable() {
    ...
    renderData(filtered);
    document.getElementById('ad-count-badge').textContent = `${filtered.length} / ${allDocs.length}`;
}

function renderData(list) {
    const tbody = document.getElementById('ad-tbody');
    const mCards = document.getElementById('ad-mobile-cards');

    if (!list.length) {
        const emptyHtml = `<div class="ad-empty">
    ...
    tbody.innerHTML = list.map(d => {
        ...
        let actions = `<div class="ad-actions" style="justify-content: flex-end;">
            <a href="${window.BASE_URL}/api/documents/view?id=${d.id}" target="_blank" class="ad-btn ad-btn-view" title="<?= __('admin_docs_btn_view') ?>">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
            </a>`;
            
        if (d.status === 'pending') {
            actions += `
                <button onclick="updateStatus(${d.id}, 'approved')" class="ad-btn ad-btn-approve" title="<?= __('admin_docs_btn_approve') ?>">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                </button>
                <button onclick="updateStatus(${d.id}, 'rejected')" class="ad-btn ad-btn-reject" title="<?= __('admin_docs_btn_reject') ?>">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>`;
        }
        actions += `</div>`;

        return `<tr class="ad-anim-in">
            <td>
            ...
            <td><span class="ad-status-badge status-${d.status}">${d.status}</span></td>
            <td>${actions}</td>
        </tr>`;
    }).join('');
    ...
    mCards.innerHTML = list.map(d => {
        ...
        let actions = `
            <a href="${window.BASE_URL}/api/documents/view?id=${d.id}" target="_blank" class="ad-btn ad-btn-view" style="flex:1; gap:6px;">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
        ...
        if (d.status === 'pending') {
            actions += `
                <button onclick="updateStatus(${d.id}, 'approved')" class="ad-btn ad-btn-approve" style="flex:1;">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                </button>
                <button onclick="updateStatus(${d.id}, 'rejected')" class="ad-btn ad-btn-reject" style="flex:1;">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>`;
        }
        
        return `<div class="ad-card ad-anim-in">
            <div class="ad-card-top">
                <div>
                    <div class="ad-card-title">${escHtml(d.full_name)}</div>
                    <div class="ad-card-sub">${escHtml(d.email)}</div>
                </div>
                <span class="ad-status-badge status-${d.status}">${d.status}</span>
            </div>
